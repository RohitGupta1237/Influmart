    ...
    const getData = async () => {
      ...
      if (!id && !clickedId) {
        ...
        getSocialData(getId, showAlert)
          .then((data) => {
            console.log(JSON.parse(data.ytData).highlights)
            const transformedFb = transformFB(data);
            const transformedYt = transformYT(JSON.parse(data.ytData).analytics);
            ...
            let popular =[]
            const popularYtPosts = JSON.parse(data.ytData).highlights;
